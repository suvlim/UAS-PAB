...
public class EditLahan extends AppCompatActivity {
    Realm realm;
    int lahanId;
    Lahan lahan;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lahanId = getIntent().getIntExtra("IdLahan", -1);
        ...
        btn_simpan.setOnClickListener(view -> {
            ...
            if (!validasiKelengkapanData() || !validasiKelembapan(kelembapanTanah)) {
                return;
            }

            // Update Data ke Realm
            realm.executeTransactionAsync(r -> {
                Lahan update = r.where(Lahan.class).equalTo("id", lahanId).findFirst();
                if (update != null) {
                    update.setJenisTanaman(jenisTanaman);
                    update.setTanggalTanam(tanggalMulaiTanam);
                    update.setEstimasiPanen(tanggalEstimasiPanen);
                    update.setKelembapanTanah(kelembapanTanah);
                }
            }, () -> {
                // onSuccess
                runOnUiThread(() -> {
                    Toast.makeText(this, "Data Tanam Baru Berhasil Disimpan!", Toast.LENGTH_SHORT).show();
                    toPengaturanLahan();
                });
            }, error -> {
                // onError
    ...
    private boolean validasiKelengkapanData() {
